...
## エージェント機能（researchエージェントから統合）

このスキルは`context: fork`設定によりサブエージェントとして実行され、以下の専門機能を持つ。

### Issue分析

- Issueのタイトル、本文、ラベル、マイルストーンをパース
- 関連Issueやリンクを抽出

### コードベース調査

- Issueで言及されているファイルやモジュールを特定
- 関連するコードパターンを検索
- 既存の類似実装を発見
...
- 関連するテストファイルを特定
- 調査方法:
  - `package.json` / `requirements.txt`から外部依存関係をパース
  - import/require文を分析して内部依存関係を追跡
  - 設定ファイル（tsconfig.json、webpack.config.js等）をチェック
...
- 読み取り専用（調査中はコードを変更しない）
- 機密情報（.env、credentials等）は読み取らない
- **調査制限**（大規模コードベース用）:
  - 最大調査時間: 10分
  - 詳細分析する最大ファイル数: 50ファイル
  - 制限に達した場合、それまでの調査結果を文書化し、残りの調査項目を記録
