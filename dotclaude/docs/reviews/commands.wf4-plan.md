...
実装計画（02_PLAN.md）を作成するコマンドの仕様書。Specを分析し、詳細なコードベース調査を行った上で、実装ステップに分割する。各ステップは1回のwf6-implement実行に対応し、依存関係とリスクを考慮した順序で構成される。ステップサイズのガイドライン（50-200行、1-5ファイル）も明記されている。
