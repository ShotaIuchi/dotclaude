...
本ドキュメントは、wf5-implementコマンドのサポートとして動作するimplementerエージェントの定義ファイルである。02_PLAN.mdに基づいて実装計画の1ステップを実行し、コード変更・テスト実行・ログ更新を担当する。ワークフロー管理システムにおける実装フェーズの中核を担う重要なエージェントである。
