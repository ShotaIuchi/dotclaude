...
ghwf_invoke_claude() {
    ...
    if [ -n "$instruction" ]; then
        ...
        ghwf_retry "$max_retries" "$retry_delay" 2 bash -c "echo '$instruction' | claude --print '/$cmd revise' 2>&1 | tee '$log_file'" || result=$?
    else
        # New execution: ghwf1-kickoff requires issue number
        if [ "$step" -eq 1 ]; then
            ghwf_retry "$max_retries" "$retry_delay" 2 bash -c "claude --print '/$cmd $issue_number' 2>&1 | tee '$log_file'" || result=$?
        else
            ghwf_retry "$max_retries" "$retry_delay" 2 bash -c "claude --print '/$cmd' 2>&1 | tee '$log_file'" || result=$?
        fi
    fi
