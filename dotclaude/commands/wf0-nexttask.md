...
if [ "$remaining" -eq 0 ]; then
  ...
  next_ready=$(echo "$schedule" | jq -r '
    .works | to_entries
    | map(select(.value.status == "pending"))
    | map(select(
        (.value.dependencies | length == 0) or
        (all(.value.dependencies[]; . as $dep | $parent.works[$dep].status == "completed"))
      ))
    | .[].key
  ' --from-file /dev/stdin <<< "$schedule" 2>/dev/null)

  if [ -n "$next_ready" ]; then
    echo "  Ready:"
    for w in $next_ready; do
      echo "    - $w"
    done
  fi

  blocked=$(echo "$schedule" | jq -r '
    .works | to_entries
    | map(select(.value.status == "pending"))
    | map(select(
