...
`/doc-review`コマンドは、ドキュメントファイルのレビューを作成し、`reviews/README.<path>.<filename>.md`形式で出力するコマンドである。単一ファイルおよび複数ファイルの処理に対応し、複数ファイル処理時は`doc-reviewer`サブエージェントを用いた並列実行をサポートする。
